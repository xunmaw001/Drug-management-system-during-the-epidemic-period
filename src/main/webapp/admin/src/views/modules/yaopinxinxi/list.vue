<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yaopinxinxi','药品购买') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yaopingoumaiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'药品购买':'' }}</el-button>
                <el-button v-if="isAuth('yaopinxinxi','药品购买') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yaopingoumaiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'药品购买':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yaopinxinxi','药品购买') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yaopingoumaiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'药品购买':'' }}</el-button>
    ...
    <yaopingoumai-cross-add-or-update v-if="yaopingoumaiCrossAddOrUpdateFlag" :parent="this" ref="yaopingoumaiCrossaddOrUpdate"></yaopingoumai-cross-add-or-update>
...
import yaopingoumaiCrossAddOrUpdate from "../yaopingoumai/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      yaopingoumaiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    yaopingoumaiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yaopingoumaiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.yaopingoumaiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.yaopingoumaiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.leixingOptions = "非处方药,处方药".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
